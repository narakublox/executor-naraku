...
_G.loadstring = function(code, chunkName)
    ...
    if status and func then return func end
    
    -- Fallback jika loadstring standard dibatasi environment-nya
    if EXT_loadstring then 
        return EXT_loadstring(code)
    end
    return nil, "Loadstring bypass failed"
end
...
-- ============================================================================
-- SEKTOR 2: KONFIGURASI SCRIPTBOX & TITLEBOX (FONT REMOVED)
-- ============================================================================
...
if ScriptBox then
    ...
    ScriptBox.Focused:Connect(function()
        if ScriptBox.Text == SCRIPTBOX_PLACEHOLDER then
            ScriptBox.Text = ""
            placeholderState.scriptBoxHasPlaceholder = false
        end
    end)
    
    -- Kembalikan placeholder jika kosong saat ditinggal (FocusLost)
    ScriptBox.FocusLost:Connect(function(enterPressed)
        local currentText = ScriptBox.Text:match("^%s*(.-)%s*$") or ""
        if currentText == "" then
...
if TitleBox then
    ...
    TitleBox.Focused:Connect(function()
        if TitleBox.Text == TITLEBOX_PLACEHOLDER then
            TitleBox.Text = ""
    ...
    TitleBox.FocusLost:Connect(function(enterPressed)
        local currentText = TitleBox.Text:match("^%s*(.-)%s*$") or ""
        if currentText == "" then
...
-- ============================================================================
-- SEKTOR 7: LOADSTRING EXECUTOR WITH ENVIRONMENT FIXED
-- ============================================================================
...
local function executeScript(scriptContent, scriptName)
    ...
    -- Menggunakan environment global utama agar fungsi raw loadstring berjalan lancar
    local customEnv = setmetatable({}, {__index = getfenv(0) or _G})
    
    customEnv.loadstring = _G.loadstring
    customEnv.require = _G.require
    customEnv.script = Instance.new("Script")
    customEnv.script.Name = scriptName or "Direct Script"
    
    -- Eksekusi utama menggunakan standard executor environment
    local loadedFunc, loadError = _G.loadstring(scriptContent, "=[" .. (scriptName or "NarakuBlox") .. "]")
    
    if not loadedFunc then
        -- Alternatif eksekusi langsung jika loadstring kustom mengembalikan nil
        local directSuccess, directError = pcall(function()
            local rawFunc = loadstring(scriptContent)
            if rawFunc then 
                setfenv(rawFunc, customEnv)
                rawFunc() 
            else
                error("Raw compile failed")
            end
        end)
...
local function refreshLocalScripts()
    ...
    for _, filePath in ipairs(files) do
        ...
        -- Double click atau click untuk edit tanpa terganggu placeholder
...
-- ============================================================================
-- SEKTOR 14: CLEAR BUTTON (AUTO RESET PLACEHOLDER)
-- ============================================================================
...
-- ============================================================================
-- SEKTOR 15: SCRIPTBOX INPUT VALIDATION (OPTIONAL)
-- ============================================================================
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode.S and UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) then
        if AddScriptButton then
            task.spawn(function()
                -- Mengaktifkan fungsionalitas tombol simpan secara langsung
                local titleText = (TitleBox.Text or ""):match("^%s*(.-)%s*$") or ""
                local scriptText = ScriptBox.Text or ""
                if titleText ~= "" and titleText ~= TITLEBOX_PLACEHOLDER and scriptText ~= "" and scriptText ~= SCRIPTBOX_PLACEHOLDER then
                    local safeTitle = titleText:gsub("[/<>:\"|?*\\]", "_"):gsub("%s+", "_")
                    local filePath = DELTA_FOLDER .. "/" .. safeTitle .. ".lua"
                    if writefile then writefile(filePath, scriptText) refreshLocalScripts() end
                end
            end)
        end
    end
    ...
    if input.KeyCode == Enum.KeyCode.E and UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) then
        local scriptText = ScriptBox.Text or ""
        if scriptText ~= "" and scriptText ~= SCRIPTBOX_PLACEHOLDER then
            executeScript(scriptText, "Direct Script")
        end
    end
end)
