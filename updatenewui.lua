...
-- =============================================================================
-- ULTIMATE MAIN PANEL CORE ENGINE (100% PERFECT FIXED - ANTI LIEUR)
-- =============================================================================
...
local CloseButton   = LMG2L["CloseButton_7f"]    

-- Sub-Panel
local PanelMain     = LMG2L["PanelMain_6a"]      
local PanelMenu     = LMG2L["PanelMenu_4c"]      
local PanelInfo     = LMG2L["PanelInfo_19"]      
local PanelUpload   = LMG2L["PanelUpload_7"]     
local UploadButton  = LMG2L["UploadButton_78"]    
...
-- Simpan Warna Asli Tombol Samping
local originalMainColor, originalMainTrans = MainButton.BackgroundColor3, MainButton.BackgroundTransparency
local originalMenuColor, originalMenuTrans = MenuButton.BackgroundColor3, MenuButton.BackgroundTransparency
local originalInfoColor, originalInfoTrans = InfoButton.BackgroundColor3, InfoButton.BackgroundTransparency

-- =============================================================================
-- FUNGSI UTAMA: RESET & BUKA PANEL (SABERS - SAPU BERSIH)
-- =============================================================================
local function resetSemuaPanel()
...
-- =============================================================================
-- EVENT LISTENERS (KLIK BUTTON)
-- =============================================================================

-- KLIK MAIN BUTTON -> PANEL MAIN KELUAR / KEBOKA!
MainButton.MouseButton1Click:Connect(function()
    playClickSound()
    bukaTab("Main")
end)

-- KLIK MENU BUTTON -> PANEL MENU KEBUKA!
MenuButton.MouseButton1Click:Connect(function()
    playClickSound()
    bukaTab("Menu")
end)

-- KLIK INFO BUTTON -> PANEL INFO KEBUKA!
InfoButton.MouseButton1Click:Connect(function()
    playClickSound()
    bukaTab("Info")
end)

-- KLIK UPLOAD BUTTON -> TOGGLE PANEL UPLOAD (MUNCUL / SEMBUNYI)
...
-- =============================================================================
-- KONDISI AWAL SAAT DI-EXECUTE
-- =============================================================================
PanelUtama.Visible = true
PanelUtama.BackgroundTransparency = 0.2
OpenButton.Visible = false

-- SAAT DI-EXECUTE, LANGSUNG PAKSA BUKA PANEL MAIN BIAR GAK KOSONG DAN GAK NUMPUK!
bukaTab("Main")

-- =============================================================================
-- LOGIKA CLOSE & OPEN BUTTON + DRAGGING + GRADIENT (TETAP AMAN)
-- =============================================================================
CloseButton.MouseButton1Click:Connect(function()
    playClickSound()
    resetSemuaPanel()
    
    local fadeTween = TweenService:Create(PanelUtama, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 1})
    fadeTween:Play()
    fadeTween.Completed:Connect(function()
        PanelUtama.Visible = false
        OpenButton.Visible = true
        OpenButton.Size = UDim2.new(0, 0, 0, 0)
        TweenService:Create(OpenButton, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0, 35, 0, 35)}):Play()
    end)
end)

OpenButton.MouseButton1Click:Connect(function()
    playClickSound()
    local shrinkOpen = TweenService:Create(OpenButton, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Size = UDim2.new(0, 0, 0, 0)})
    shrinkOpen:Play()
    shrinkOpen.Completed:Connect(function()
        ...
        local openTween = TweenService:Create(PanelUtama, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 0.2})
        openTween:Play()
        openTween.Completed:Connect(function()
            bukaTab("Main") -- Balik otomatis ke Main pas dibuka lagi
        end)
    end)
end)
...
-- Gradient Loop
local strokeGradients, buttonShineGradients = {}, {}
local function classifyGradients(object)
    for _, desc in ipairs(object:GetDescendants()) do
        if desc:IsA("UIGradient") then
            if desc.Parent and desc.Parent:IsA("UIStroke") then table.insert(strokeGradients, desc)
            elseif desc.Parent and (desc.Parent:IsA("ImageButton") or desc.Parent:IsA("TextButton")) then table.insert(buttonShineGradients, desc) end
        end
    end
end
...
task.spawn(function()
    while ScreenGui and ScreenGui.Parent do
        for i = 1, #strokeGradients do
            ...
            if grad and grad.Parent then grad.Rotation = (grad.Rotation + 2) % 360 end
        end
        task.wait(0.02)
    end
end)
...
task.spawn(function()
    for _, grad in ipairs(buttonShineGradients) do grad.Offset = Vector2.new(-1, 0) end
    while ScreenGui and ScreenGui.Parent do
        for progress = -100, 100, 4 do
            ...
            for i = 1, #buttonShineGradients do
                ...
                if grad and grad.Parent then grad.Offset = Vector2.new(currentOffset, 0) end
            end
           task.wait(0.02)
        end
        task.wait(1.5)
...
print("NARAKU BLOX FIXED TOTAL - NO MORE OVERLAP!")
