...
-- =============================================================================
-- ULTIMATE MAIN PANEL CORE ENGINE (100% PERFECT CLEAN - FIXED HIERARCHY OVERLAP)
-- =============================================================================
...
-- 2. Inisialisasi Referensi Objek Sesuai Struktur UI (Murni Sesuai File updatenewui.txt)
local ScreenGui     = LMG2L["ScreenGui_1"]       -- ScreenGui Utama
local OpenButton    = LMG2L["OpenButton_2"]      -- Tombol Buka Panel
local PanelUtama    = LMG2L["PanelUtama_6"]      -- Frame Vertikal Utama (Sidebar)
...
-- Konten Sub-Panel Samping (Anak dari PanelUtama)
local PanelMain     = LMG2L["PanelMain_6a"]      -- Panel Fitur Main
...
-- Sub-Panel Upload Script (Anak dari PanelMain)
local PanelUpload   = LMG2L["PanelUpload_7"]     
local UploadButton  = LMG2L["UploadButton_78"]    -- Tombol Trigger Upload (Di dalam PanelMain)
...
local originalMainColor = MainButton.BackgroundColor3
local originalMainTrans = MainButton.BackgroundTransparency

local originalMenuColor = MenuButton.BackgroundColor3
local originalMenuTrans = MenuButton.BackgroundTransparency

local originalInfoColor = InfoButton.BackgroundColor3
local originalInfoTrans = InfoButton.BackgroundTransparency

-- 4. Pengaturan Kondisi Awal Visibilitas UI (Sapu Bersih Semua Child Sejak Awal)
PanelUtama.Visible = true
PanelUtama.BackgroundTransparency = 0.2

-- Matikan semua sub-panel anak biar gak numpuk di frame pertama
...
-- 5. Fungsi Mengatur Perpindahan Visual Warna Aktif Antara Main, Menu, & Info
local function updateTabVisual(activeTab)
    local tweenInfo = TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    
    -- Reset semua tombol ke warna asli terlebih dahulu
    TweenService:Create(MainButton, tweenInfo, {BackgroundColor3 = originalMainColor, BackgroundTransparency = originalMainTrans}):Play()
    TweenService:Create(MenuButton, tweenInfo, {BackgroundColor3 = originalMenuColor, BackgroundTransparency = originalMenuTrans}):Play()
    TweenService:Create(InfoButton, tweenInfo, {BackgroundColor3 = originalInfoColor, BackgroundTransparency = originalInfoTrans}):Play()
    
    -- Berikan efek highlight putih transparan pada tab yang sedang aktif terbuka
    if activeTab == "Main" then
        TweenService:Create(MainButton, tweenInfo, {BackgroundColor3 = Color3.fromRGB(255, 255, 255), BackgroundTransparency = 0.5}):Play()
    elseif activeTab == "Menu" then
        TweenService:Create(MenuButton, tweenInfo, {BackgroundColor3 = Color3.fromRGB(255, 255, 255), BackgroundTransparency = 0.5}):Play()
    elseif activeTab == "Info" then
        TweenService:Create(InfoButton, tweenInfo, {BackgroundColor3 = Color3.fromRGB(255, 255, 255), BackgroundTransparency = 0.5}):Play()
    end
end

-- 6. FIX LOGIKA TOTAL TAB SELECTOR (MURNI TOGGLE VISIBILITY - ANTI-NUMPUK 100%)
local function toggleTab(targetTab)
    -- JIKA KLIK TOMBOL YANG SAMA SAAT PANEL SEDANG TERBUKA -> TUTUP TOTAL PANEL TERSEBUT
    if currentActiveTab == targetTab then
        if PanelMain then PanelMain.Visible = false end
        if PanelMenu then PanelMenu.Visible = false end
        if PanelInfo then PanelInfo.Visible = false end
        if PanelUpload then PanelUpload.Visible = false end
    ...
    -- Sembunyikan SEMUA sub-panel anak tanpa terkecuali sebelum membuka yang baru
    if PanelMain then PanelMain.Visible = false end
    if PanelMenu then PanelMenu.Visible = false end
    if PanelInfo then PanelInfo.Visible = false end
    if PanelUpload then PanelUpload.Visible = false end

    -- Daftarkan tab yang baru diklik sebagai tab aktif
    currentActiveTab = targetTab
    updateTabVisual(targetTab)

    -- Munculkan murni panel anak yang dituju secara instan & bersih
    if targetTab == "Main" then
        if PanelMain then PanelMain.Visible = true end
    elseif targetTab == "Menu" then
        if PanelMenu then PanelMenu.Visible = true end
    elseif targetTab == "Info" then
        if PanelInfo then PanelInfo.Visible = true end
    end
end

-- Set default awal: Tampilkan panel Main langsung di awal eksekusi script
toggleTab("Main")

-- 7. Animasi Flash Sekali Kedip untuk Feedback Klik
local function applyFlashEffect(button)
    if not button then return end
    local oldColor = button.BackgroundColor3
    local oldTrans = button.BackgroundTransparency
...
-- 8. Sistem Dragging Manual yang Stabil untuk OpenButton (Mobile & PC Friendly)
...
-- 9. Logika Interaksi Menggunakan Animasi Fade-Out & Pop-Up TOTAL SIDEBAR PANEL
CloseButton.MouseButton1Click:Connect(function()
    playClickSound()
    applyFlashEffect(CloseButton)
    
    -- Sembunyikan konten anak-anaknya dulu biar bersih total dan gak melayang
    if PanelMain then PanelMain.Visible = false end
    if PanelMenu then PanelMenu.Visible = false end
    if PanelInfo then PanelInfo.Visible = false end
    if PanelUpload then PanelUpload.Visible = false end
    
    -- Animasi menyusut/hilang halus khusus Sidebar Utama (PanelUtama)
...
OpenButton.MouseButton1Click:Connect(function()
    ...
    shrinkOpen.Completed:Connect(function()
        ...
        -- Otomatis load ulang keadaan awal ke Tab Main
...
-- 10. Logika Tombol Navigasi Samping (Menu, Main, Info)
...
-- 11. FIX STRUKTUR: Logika Click UploadButton (Toggle Mandiri dari Anak PanelMain)
if UploadButton and PanelUpload then
    UploadButton.MouseButton1Click:Connect(function()
        playClickSound()
        applyFlashEffect(UploadButton)
        
        -- Mengubah status visibilitas murni milik PanelUpload
        PanelUpload.Visible = not PanelUpload.Visible
    end)
end

-- 12. Klasifikasi Efek Gradien (Looping Neon Border & Efek Shine Mengkilap Button)
...
-- Loop Thread 1: Efek Rotasi Rainbow/Neon Berputar pada Garis Border (UIStroke)
task.spawn(function()
...
-- Loop Thread 2: Efek Refleksi Cahaya Mengkilap Bergerak Mulus Pada Permukaan Tombol
task.spawn(function()
...
print("NARAKU BLOX NEW UI ENGINE 100% PERFECT CLEAN FIXED!")
-- =============================================================================
